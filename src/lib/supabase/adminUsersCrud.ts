...
function rowToManagedUser(
  ...
  return {
    ...
    role,
    areaIds,
    canImport: Boolean(raw.can_import),
...
export async function updateAdminUser(
  ...
  if (myAuthId && targetAuth === myAuthId) {
    const danger = input.isActive === false || input.role !== "admin"
    if (danger) {
      return {
  ...
  const sync = await syncUserAreas(input.profileId, input.areaIds)
  ...
  if (targetAuth) {
    ...
    const patch: {
      ...
      user_metadata: { name: input.name.trim(), app_role: input.role },
      ban_duration: input.isActive ? "0" : "876600h",
    }
...
export async function activateAdminUser(
  ...
  const { error: upErr } = await supabase
    .from("users")
    .update({ auth_user_id: uid, password_hash: null })
    .eq("id", profileId)
